...
For node \lstinline{i}, \lstinline{arridx[i]} is the array index for range queries (in the range $(0,n]$), 
\lstinline{hchild[i]} is the heavy child (or $-1$ for leaf nodes), and
\lstinline{hpLeaf[i]} and \lstinline{hpRoot[i]} are the leaf and root nodes respectively of the heavy path passing through node \lstinline{i}.

\lstinline{lca(a, b)} returns the lca of a and b (in $\mathcal{O}(\log(n))$).
If \lstinline{intv} is not null, \lstinline{*intv} will receive up to
$2\log_2(n)$ intervals such that for all nodes \lstinline{x} on the path between \lstinline{a} and \lstinline{b},
there is exactly one \lstinline{i} where \lstinline{intv[i].first <= arridx[x] < intv[i].second}.\\
If \lstinline{intvIncludeLCA} is false, the lca of a and b will not be included in these intervals.

\lstinputlisting[language=cpp]{code/hld.cpp}
